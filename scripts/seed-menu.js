/**
 * Script seeding dữ liệu merchant menu
 * Sử dụng MongoDB client trực tiếp
 * Chạy bằng: node scripts/seed-menu.js <user_id> <email>
 */
...
async function seedMenuData() {
  ...
  if (!userId || !userEmail) {
    console.error('Usage: node scripts/seed-menu.js <user_id> <email>');
    console.error('Example: node scripts/seed-menu.js 698d3ca5001a035ba0dce744 votrungkien240705@gmail.com');
    process.exit(1);
  }
  ...
  try {
    ...
    if (!merchant) {
      ...
      const result = await db.collection('merchants').insertOne(merchantData);
      merchant = await db.collection('merchants').findOne({ _id: result.insertedId });
    }
